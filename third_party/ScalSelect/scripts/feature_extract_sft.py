...
- hybrid_meta: Fast hidden/text/metadata representation, used by default
- qwen_attention: Original Qwen user-attention baseline, enabled by --feature-mode

Outputs:
- Sample representations: Saved to NPZ format
...
DEFAULT_FEATURE_DIR = os.environ.get("MITS_FEATURE_DIR", os.path.join(DEFAULT_WORK_DIR, "features"))
...
def process_image_paths(images: List[str], base_path: str = None) -> List[str]:
    ...
    for img in images:
        ...
        if os.path.isabs(img):
            processed.append(os.path.normpath(img))
        elif base_path:
            processed.append(os.path.normpath(os.path.join(base_path, img)))
        else:
            processed.append(os.path.normpath(img))
    return processed
...
def process_sample_batch(
    ...
    For each sample:
    - hybrid_meta: use the extractor's combined representation directly.
    - qwen_attention: compute the original user-attended vision representation.
    ...
    for sample in samples:
        ...
        images = process_image_paths(images, base_path=image_base_path)
        ...
        sample_meta = dict(sample.get('meta', {}) or {})
        sample_meta['images'] = images
        sample_meta_list.append(sample_meta)
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max(1, image_load_workers)) as executor:
        ...
        valid_meta = [sample_meta_list[i] for i in valid_indices]
        ...
        for idx, features in enumerate(batch_features):
            ...
            try:
                if getattr(extractor, 'feature_mode', 'qwen_attention') != 'qwen_attention':
                    combined_repr = features.get('combined_repr')
                    if combined_repr is None:
                        results_map[sample_id] = None
                        continue
                    if isinstance(combined_repr, torch.Tensor):
                        combined_repr = combined_repr.detach().cpu().numpy()
                    elif not isinstance(combined_repr, np.ndarray):
                        combined_repr = np.asarray(combined_repr, dtype=np.float32)
        ...
        if 'valid_images' in locals():
            del valid_images
        if 'valid_meta' in locals():
            del valid_meta
        if 'results_map' in locals():
            del results_map

        if getattr(extractor, "cleanup_cuda_cache", False) and torch.cuda.is_available():
...
def main():
    ...
    parser.add_argument(
        ...
        help=f'Model name or path (default: {DEFAULT_MODEL_PATH})',
    )
    parser.add_argument(
        '--dataset',
        type=str,
        default=DEFAULT_SHAREGPT_PATH,
        help=f'Path to ShareGPT format JSON dataset (default: {DEFAULT_SHAREGPT_PATH})',
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default=DEFAULT_FEATURE_DIR,
        help=f'Output directory for extracted features (default: {DEFAULT_FEATURE_DIR})',
    )
    parser.add_argument(
        '--max-samples',
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Number of samples to process together (default: 1)',
    )
    parser.add_argument(
        '--torch-dtype',
        type=str,
    ...
    parser.add_argument(
        ...
        default='qwen',
        choices=['llava', 'qwen'],
        help='Model type: llava or qwen (default: qwen)',
    )
    parser.add_argument(
        '--feature-mode',
        type=str,
        default='hybrid_meta',
        choices=['hybrid_meta', 'qwen_attention'],
        help='Feature backend: hybrid_meta is fast default; qwen_attention keeps the original baseline.',
    )
    parser.add_argument(
        '--text-alpha',
        type=float,
        default=0.30,
        help='Hybrid mode weight for hashed conversation text features (default: 0.30).',
    )
    parser.add_argument(
        '--meta-alpha',
        type=float,
        default=0.15,
        help='Hybrid mode weight for hashed MITS metadata features (default: 0.15).',
    )
    parser.add_argument(
        '--image-base-path',
        type=str,
        default=None,
        help='Optional base path for relative image paths. Absolute image paths are left unchanged.',
    )
    parser.add_argument(
        '--image-load-workers',
        type=int,
        default=4,
        help='Number of CPU threads used to load images per batch (default: 4).',
    )
    parser.add_argument(
        '--qwen-image-min-pixels',
        type=int,
        default=None,
        help='Qwen image processor minimum pixel budget. Default keeps the project setting.',
    )
    parser.add_argument(
        '--qwen-image-max-pixels',
        type=int,
        default=None,
        help='Qwen image processor maximum pixel budget. Lower values reduce vision tokens and speed up extraction.',
    )
    parser.add_argument(
        '--disable-base-model-hidden',
        action='store_true',
        help='Disable the hybrid_meta fast path that skips LM-head logits.',
    )
    parser.add_argument(
        '--cleanup-cuda-cache',
        action='store_true',
        help='Call torch.cuda.empty_cache() after batches. Slower, but useful if memory fragments or OOM occurs.',
    )
    ...
    if accelerator.is_main_process:
        ...
        print(f"Text alpha: {args.text_alpha}")
        print(f"Meta alpha: {args.meta_alpha}")
        print(f"Image base path: {args.image_base_path or '(none; use dataset paths as-is)'}")
        print(f"Image load workers: {args.image_load_workers}")
        print(f"Qwen image min pixels: {args.qwen_image_min_pixels or '(default)'}")
        print(f"Qwen image max pixels: {args.qwen_image_max_pixels or '(default)'}")
        print(f"Base model hidden fast path: {not args.disable_base_model_hidden}")
        print(f"Cleanup CUDA cache per batch: {args.cleanup_cuda_cache}")
    ...
    with accelerator.split_between_processes(samples) as local_samples:
        local_samples = list(local_samples)
        if accelerator.is_main_process:
        ...
        extractor = VLMFeatureExtractor(
            ...
            qwen_image_min_pixels=args.qwen_image_min_pixels,
            qwen_image_max_pixels=args.qwen_image_max_pixels,
            use_base_model_hidden=not args.disable_base_model_hidden,
            cleanup_cuda_cache=args.cleanup_cuda_cache,
        )

        # Process samples
        print(f"GPU {accelerator.process_index}: Starting to process {len(local_samples)} samples")
    ...
    if accelerator.is_main_process:
        ...
        metadata = {
            ...
            'description': (
                'Hybrid last-layer vision plus hashed text/meta representation'
                if args.feature_mode == 'hybrid_meta'
                else 'Vision representation from user attention'
            ),
        }
