...
DEFAULT_FEATURES_DIR = os.environ.get("MITS_FEATURE_DIR", os.path.join(DEFAULT_WORK_DIR, "features"))
...
def main():
    ...
    parser.add_argument(
        ...
        help=f'Directory containing all_representations.npz (default: {DEFAULT_FEATURES_DIR})',
    )
    parser.add_argument(
        '--output',
        type=str,
        default=DEFAULT_CUR_SCORES_PATH,
        help=f'Output file for sample importance scores (default: {DEFAULT_CUR_SCORES_PATH})',
    )
    parser.add_argument(
        '--sv-threshold',
        type=float,
    ...
    # Determine output path. The MITS AutoDL default is WORK_DIR/importance_scores.jsonl.
