...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ratio", type=int, action="append", default=[20, 30], help="Target percent. Can repeat.")
...
def main() -> None:
    ...
    summaries = []
    for ratio in sorted(set(args.ratio)):
        ...
        index_path = os.path.join(args.output_dir, f"mits_selected_{ratio}_targeted.jsonl")
        sharegpt_path = os.path.join(args.output_dir, f"mits_selected_{ratio}_targeted_train32_sharegpt.jsonl")
        if not args.dry_run:
